...
class App(Tk):
    ...
    def __init__(
        ...
        Tk.__init__(
            self, None, baseName=None, className="Tk", useTk=1, sync=0, use=None
        )
        ...
        self.__canvas = Canvas(
            self, width=WORLD_WIDTH * self.scale, height=WORLD_HEIGHT * self.scale
        )
        self.__canvas.pack()
        ...
        self.noise_label = Label(
            self, text=f"Noise Distribution: {noise_type}", font=("Arial", 10, "bold")
        )
        ...
        if "g" in self.history_chars:
            if self.count_since >= self.max_count_since:
                ...
                self.noise_label.config(
                    text=f"Noise Distribution: {self.simulator.gps_noise_dist}"
                )
    ...
    def __loop(self):
        ...
        self.__canvas.create_image(
            car1.pos[0] * self.scale,
            car1.pos[1] * self.scale,
            image=self.car_blue_imgs[car1_angle + 180],
        )
        self.__canvas.create_image(
            car2.pos[0] * self.scale,
            car2.pos[1] * self.scale,
            image=self.car_red_imgs[car2_angle + 180],
        )
        ...
        if self.draw_occupancy:
            for car in [car1, car2]:
                ...
                self.__canvas.create_line(
                    car.pos[0] * self.scale,
                    car.pos[1] * self.scale,
                    car.pos[0] * self.scale,
                    (car.pos[1] - dists[0]) * self.scale,
                    fill=sensor_color,
                    width=sensor_width,
                )
                self.__canvas.create_line(
            ...
            if self.simulator.do_particle_filtering:
                for p in self.simulator.particle_filter1.particles:
                    self.__canvas.create_oval(
                        (p.pos[0] - 2) * self.scale,
                        (p.pos[1] - 2) * self.scale,
                        (p.pos[0] + 2) * self.scale,
                        (p.pos[1] + 2) * self.scale,
                        fill="red",
                    )
        ...
        if self.simulator.do_particle_filtering:
            angle_est1 = int(
                math.degrees(
                    math.atan2(
                        -self.simulator.orient_est1[1], self.simulator.orient_est1[0]
                    )
                )
            )
            self.__canvas.create_image(
                self.simulator.x_est1 * self.scale,
                self.simulator.y_est1 * self.scale,
                image=self.green_arrow_imgs[angle_est1 + 180],
            )
        ...
        if self.simulator.do_kalman_filtering:
            for car, kf_state in [
                (car1, self.simulator.kf_state1),
                (car2, self.simulator.kf_state2),
            ]:
        ...
        if self.simulator.do_particle_filtering:
            self.car1_particle_count_label.config(
                text=f"Car 1 Particle Count: {self.simulator.particle_filter1.num_particles}"
            )
            self.car2_particle_count_label.config(
                text=f"Car 2 Particle Count: {self.simulator.particle_filter2.num_particles}"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        default=50,
        type=int,
        help="Number of particles for particle filtering",
    )
    parser.add_argument(
        "-m",
        "--max_sensor_range",
        default=50,
        type=int,
        help="Maximum range of the car's sensors",
    )
    parser.add_argument(
        "-s",
        "--sensor_noise_std",
        default=0.0,
        type=float,
        help="Std dev of car's sensor noise",
    )
    parser.add_argument(
        "-gv",
        "--gps_noise_var",
        default=10.0,
        type=float,
        help="Variance of gaussian noise for GPS measurement (Kalman filter)",
    )
    parser.add_argument(
        "-gw",
        "--gps_noise_width",
        default=20,
        type=float,
        help="Width of uniformly random noise for GPS measurement (Kalman filter)",
    )
    parser.add_argument(
        "-nt",
        "--noise_type",
        default="gaussian",
        choices=["gaussian", "uniform", "laplace", "cauchy"],
        help="Noise distribution type (gaussian, uniform, laplace, cauchy)",
    )
