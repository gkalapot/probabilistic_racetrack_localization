...
class ParticleFilter:
    ...
    def initialize_particles(self):
        ...
        # BEGIN_YOUR_CODE ######################################################
        particles = []
        num_of_guesses = self.num_particles
    ...
    def filtering_and_estimation(
        self, sensor, max_sensor_range, sensor_std, evidence, delta_angle, speed
    ):
        """
        ...
        self.particles = self.filtering(
            sensor, max_sensor_range, sensor_std, evidence, delta_angle, speed
        )
    ...
    def filtering(
        ...
        for particle in self.particles:
            updated_particle = self.transition_sample(particle, delta_angle, speed)
            updated_particle.weight = self.compute_prenorm_weight(
                updated_particle, sensor, max_sensor_range, sensor_std, evidence
            )
    ...
    def compute_prenorm_weight(
        self, particle, sensor, max_sensor_range, sensor_std, evidence
    ):
        """
        ...
        dist_from_sensor = sensor(particle.pos[0], particle.pos[1], max_sensor_range)

        if self.noise_type == "gaussian":
            weight = weight_gaussian_kernel(dist_from_sensor, evidence, std=15)
        elif self.noise_type == "laplace":
            weight = weight_laplace_kernel(dist_from_sensor, evidence, scale=3)
        elif self.noise_type == "cauchy":
            weight = weight_cauchy_kernel(dist_from_sensor, evidence, scale=1)
    ...
    def transition_sample(self, particle, delta_angle, speed):
        ...
        new_x = position[0] + dx * speed
        new_y = position[1] + dy * speed

        new_pos = np.array([new_x, new_y])
        new_orient = np.array([dx, dy])

        new_particle = Particle(new_pos, new_orient)
        new_particle.add_noise(noise_type=self.noise_type)
    ...
    def weighted_sample_w_replacement(self, particles):
        ...
        for _ in range(len(particles)):
            ...
            if particle is None:
                ...
                p = Particle(
                    copy.deepcopy(particle.pos), copy.deepcopy(particle.orient)
                )
    ...
    def random_select(self):
        ...
        try:
            return self.particles[
                bisect.bisect_left(self.distribution, np.random.uniform(0, 1))
            ]
        except IndexError:
