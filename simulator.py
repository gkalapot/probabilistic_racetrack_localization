...
class Simulator:
    ...
    def update_race_progress(self):
        ...
        for i, (car, next_checkpoint) in enumerate(
            [
                (self.car1, self.car1_next_checkpoint),
            ...
            if np.linalg.norm(car.pos - checkpoint) < 30:
                if i == 0:
                    ...
                    self.car1_checkpoint_reached.append(
                        (next_checkpoint, self.car1.pos.copy())
                    )
                    self.car1_next_checkpoint = (self.car1_next_checkpoint + 1) % len(
                        self.checkpoints
                    )
                    ...
                    self.car2_checkpoint_reached.append(
                        (next_checkpoint, self.car2.pos.copy())
                    )
                    self.car2_next_checkpoint = (self.car2_next_checkpoint + 1) % len(
                        self.checkpoints
                    )
        ...
        if self.do_particle_filtering:
            ...
            error1 = self.calculate_checkpoint_error(
                self.car1.pos, np.array([self.x_est1, self.y_est1]), checkpoint
            )
            self.car1_checkpoint_errors.append(error1)

            error2 = self.calculate_checkpoint_error(
                self.car1.pos, np.array([self.x_est2, self.y_est2]), checkpoint
            )
        ...
        if self.do_kalman_filtering:
            ...
            error1 = self.calculate_checkpoint_error(
                self.car1.pos, self.kf_state1[:2], checkpoint1
            )
            self.car1_checkpoint_errors.append(error1)

            checkpoint2 = self.get_next_checkpoint(2)
            error2 = self.calculate_checkpoint_error(
                self.car2.pos, self.kf_state2[:2], checkpoint2
            )
            self.car2_checkpoint_errors.append(error2)
    ...
    def init_particles(self):
        ...
        if EXTRA_CREDIT_AVAILABLE:
            self.particle_filter1 = ParticleFilterExtra(
                self.num_particles,
                0,
                WORLD_WIDTH,
                0,
                WORLD_HEIGHT,
                noise_type=self.pf_noise_type,
            )
            self.particle_filter2 = ParticleFilterExtra(
                self.num_particles,
                0,
                WORLD_WIDTH,
                0,
            ...
            self.particle_filter1 = ParticleFilter(
                self.num_particles,
                0,
                WORLD_WIDTH,
                0,
            ...
            self.particle_filter2 = ParticleFilter(
                self.num_particles,
                0,
                WORLD_WIDTH,
                0,
    ...
    def init_kalman(self):
        ...
        self.kalman_filter1 = KalmanFilter(
            self.car1, adjusted_var, self.gps_noise_width
        )
        self.kalman_filter2 = KalmanFilter(
            self.car2, adjusted_var, self.gps_noise_width
        )
    ...
    def toggle_gps_noise_dist(self):
        ...
        current_idx = (
            noise_types.index(self.gps_noise_dist)
            if self.gps_noise_dist in noise_types
            else 0
        )
        ...
        self.pf_noise_type = (
            self.gps_noise_dist if self.gps_noise_dist != "uniform" else "gaussian"
        )
    ...
    def loop(self):
        ...
        if self.replaying:
            ...
            if self.cur_i >= 2:
                ...
                self.car1.old_vel = (
                    dp1_prev[0:2] - self.lap_data_old[self.cur_i - 4][0:2]
                )
                self.car2.vel = self.car2.pos - dp2_prev[0:2]
                self.car2.old_vel = (
                    dp2_prev[0:2] - self.lap_data_old[self.cur_i - 3][0:2]
                )
        ...
        if self.do_particle_filtering:
            ...
            self.x_est1, self.y_est1, self.orient_est1 = (
                self.particle_filter1.filtering_and_estimation(
                    sensor,
            ...
            self.x_est2, self.y_est2, self.orient_est2 = (
                self.particle_filter2.filtering_and_estimation(
                    sensor,
                    self.max_sensor_range,
                    self.sensor_std,
                    evidence2,
        ...
        if self.do_kalman_filtering:
            self.gps_measurement1 = self.car1.measure_gps(
            ...
            self.gps_measurement2 = self.car2.measure_gps(
                noise_dist=self.gps_noise_dist
            )
